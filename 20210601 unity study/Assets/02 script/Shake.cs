...
public class Shake : MonoBehaviour
{
    ...
    public IEnumerator ShakeCamera(float duration = 0.05f, float mPos = 0.03f, float mRot = 0.1f)//회전 시킬 강도 함수
    {
        //경과 시간 저장용 변수
        float passTime = 0f;
        ...
        while (passTime < duration)
        {
            ...
            Vector3 shakePos = Random.insideUnitSphere;//랜덤한 흔들림 효과 만들 때 씀
            ...
            if(shakeRotate)
            {
                //펄린 노이즈는 무규칙적인 노이즈를 어느정도 보정함
                //일관성이 있는 노이즈 형태를 가진 노이즈를 발생
                //따라서 규칙성이 있도록 보이게 함
                //지형이나 사물을 배치할 때 많이 사용되며
                //예; 오픈 필드에 있는 나무나 풀 등을 심을 때 많이 사용됨
                float noise = Mathf.PerlinNoise(Time.time * mRot, 0f);//지형 랜덤하게 그릴 때 쓴다 (그나마 규칙적이나 여전히 불규칙함)
